...
@NamedQueries({
        ...
        @NamedQuery(name = Meal.BETWEEN_HALF_OPEN, query = "SELECT m FROM Meal m WHERE m.user.id=? 3  " +
                "AND m.dateTime >=? 1 AND m.dateTime <? 2 ORDER BY m.dateTime DESC"),
})
@Entity
@Table(name = "meal", uniqueConstraints = { @UniqueConstraint(columnNames = { "date_time", "user_id" }) })
